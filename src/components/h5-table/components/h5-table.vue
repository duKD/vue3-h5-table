<template>
  ...
  >
    ...
    <section  class="table-content" :style="`transform: translateX(${distanTableX});transition:none`">
      <h5-table-row
      v-for="(item, index) in props.tableDatas"
...
const handleDom = ()=>{
  ...
  return (height:number,index:number)=>{
    ...
    //获取第一列
    const firstColumn = tabelDom?.querySelector('.table-header .first-column')||null
...
const [transformX,distanX,distanY] = useGetTransformX(tableRef, tablewidth, tablecontent);
...
watchEffect(()=>{
    if(props.tableDatas.length>=props.rowNum){
      tableHeight.value = Math.max(props.rowHeight*props.rowNum + props.headerHeight,props.minTableHeight)
    }
})
